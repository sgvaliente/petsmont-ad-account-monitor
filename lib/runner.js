import { fetchTodaysInsights, getTokenInfo, getDailyBudget, fetchAdSetInsights, fetchTodaysAccountSpend, fetchTodaysAccountCPA } from './metaClient.js';
import { 
  ruleTokenExpiration, 
...
export async function runChecks() {
  ...
  try {
    ...
    // Check if there are any active campaigns with spend today
    const hasActiveCampaigns = todaysInsights.some(adset => adset.spend > 0);
    console.log(`🔍 [RUNNER] Active campaigns check: ${hasActiveCampaigns ? 'YES' : 'NO'} (${todaysInsights.filter(a => a.spend > 0).length} ad sets with spend)`);
    
    // Send alerts if any AND there are active campaigns
    if (alerts.length > 0 && hasActiveCampaigns) {
    ...
    } else if (alerts.length > 0 && !hasActiveCampaigns) {
