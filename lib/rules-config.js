...
export const RULES_CONFIG = {
  ...
  PACING: {
    ...
    VELOCITY_THRESHOLD: 3.0,      // 2x normal spending velocity
    ACCELERATION_THRESHOLD: 1.5,   // 1.5x acceleration in spending rate
    
    // Time-based pacing checks
    HOURLY_SPEND_LIMITS: {
      9: 0.20,   // 20% by 9am (was 15%)
      10: 0.35,  // 35% by 10am (was 25%)
      11: 0.50,  // 50% by 11am (was 35%)
      12: 0.65,  // 65% by 12pm (was 45%)
      13: 0.75,  // 75% by 1pm (was 55%)
      14: 0.85,  // 85% by 2pm (was 65%)
      15: 0.90,  // 90% by 3pm (was 75%)
      16: 0.95,  // 95% by 4pm (was 85%)
      17: 1.00,  // 100% by 5pm (was 95%)
    },
...
export function shouldSendSummaryReport() {
  ...
  // Get current EST time to check minutes
  const now = new Date();
  const estTime = new Date(now.toLocaleString("en-US", {timeZone: "America/New_York"}));
  const currentMinutes = estTime.getMinutes();
  
  // Check if current hour matches any report hour AND we're in the first 30 minutes
  // This ensures summaries are only sent once per hour (cron runs every 30 minutes)
  const isReportHour = REPORT_HOURS.includes(dayProgress.currentHour);
  const shouldSend = isReportHour && currentMinutes < 30;
