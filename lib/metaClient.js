...
export async function getDailyBudget() {
  try {
    ...
    // Get campaign budgets to estimate daily budget
    ...
    for (const campaign of campaigns) {
      console.log(`📊 [META] Campaign: ${campaign.name} (${campaign.status})`);
    ...
    console.log(`📊 [META] Total daily budget: $${totalDailyBudget.toFixed(2)} (from ${activeCampaigns} active campaigns)`);
