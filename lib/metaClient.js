...
export async function fetchAdSetInsights({ since, until }) {
  ...
  try {
    ...
    // Customize fields as needed - including cost_per_purchase for CPA calculation
    const fields = ['adset_id','adset_name','spend','impressions','clicks','ctr','cpc','cost_per_purchase','actions'];
    const params = {
    ...
    const processedInsights = insights.map(i => {
      ...
      if (i.actions && Array.isArray(i.actions)) {
        const purchaseAction = i.actions.find(action => action.action_type === 'purchase');
      ...
      return {
        ...
        ctr: Number(i.ctr || 0),
        cpc: Number(i.cpc || 0),
        costPerPurchase: Number(i.cost_per_purchase || 0),
        purchases: purchases,
        actions: i.actions || []
      };
    });
...
/**
 * Fetch today's account-level cost per purchase (EST timezone)
 * This gives us the true CPA for the account
 */
export async function fetchTodaysAccountCPA() {
  const now = new Date();
  ...
  console.log(`📊 [META] Fetching account-level CPA for EST date: ${todayStr}`);
  
  try {
    const account = new AdAccount(config.adAccountId);
    const insights = await account.getInsights(['spend', 'actions'], {
      time_range: { since: todayStr, until: todayStr },
      level: 'account'
    });
    
    if (insights.length === 0) {
      console.log(`📊 [META] No account-level insights found`);
      return 0;
    }
    ...
    // Find purchase action in the actions array
    let totalPurchases = 0;
    if (insight.actions && Array.isArray(insight.actions)) {
      const purchaseAction = insight.actions.find(action => action.action_type === 'purchase');
      totalPurchases = purchaseAction ? Number(purchaseAction.value || 0) : 0;
    }
    
    const cpa = totalPurchases > 0 ? totalSpend / totalPurchases : 0;
    console.log(`📊 [META] Account-level CPA calculation: $${totalSpend} spend ÷ ${totalPurchases} purchases = $${cpa.toFixed(2)}`);
    
    return cpa;
  } catch (error) {
    console.error('❌ [META] Error fetching account-level CPA:', error.message);
    throw error;
  }
}
 ...
 */
export async function hasActiveCampaigns() {
  try {
    console.log(`📊 [META] Checking for currently active campaigns...`);
    const account = new AdAccount(config.adAccountId);
    
    // Get all campaigns with their current status
    const campaigns = await account.getCampaigns(['status', 'name']);
    
    const activeCampaigns = campaigns.filter(campaign => campaign.status === 'ACTIVE');
    
    console.log(`📊 [META] Total campaigns: ${campaigns.length}`);
    console.log(`📊 [META] Active campaigns: ${activeCampaigns.length}`);
    
    if (activeCampaigns.length > 0) {
      console.log(`📊 [META] Active campaign names:`, activeCampaigns.map(c => c.name));
    }
    
    return activeCampaigns.length > 0;
  } catch (error) {
    console.error('❌ [META] Error checking active campaigns:', error.message);
    // Default to true to be safe (don't skip alerts if we can't check)
    return true;
  }
}
  ...
  try {
    ...
    for (const campaign of campaigns) {
      ...
      if (isActive || hasSpendToday) {
        ...
        if (campaign.daily_budget && campaign.daily_budget !== '0') {
        ...
        } else if (campaign.lifetime_budget && campaign.lifetime_budget !== '0') {
          ...
          console.log(`📊 [META] No budget found for campaign: ${campaign.name} (daily_budget: ${campaign.daily_budget}, lifetime_budget: ${campaign.lifetime_budget})`);
        }
      }
    }
