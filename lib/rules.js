 ...
 */
export function ruleCPAAboveTarget(accountCPA) {
  const alerts = [];
  ...
  if (isCPAAboveTarget(accountCPA)) {
    alerts.push({
      ...
      detail: `$${accountCPA.toFixed(0)} CPA (target: $${RULES_CONFIG.CPA.TARGET_CPA}) - ${((accountCPA / RULES_CONFIG.CPA.TARGET_CPA) * 100).toFixed(0)}% above target`,
      severity: RULES_CONFIG.SEVERITY.WARNING
    });
  }
